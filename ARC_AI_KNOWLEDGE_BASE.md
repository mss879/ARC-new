# ARC AI Agency - Master Knowledge Base

This document contains a comprehensive, structured compilation of all information regarding **ARC AI Agency (Pvt) Ltd**. It is designed to be easily ingested and utilized by AI agents, chatbots, and RAG systems as their single source of truth.

---

## 1. Company Profile & Trust Facts

### Brand Positioning
ARC AI helps businesses grow through premium websites, custom AI systems, workflow automation, and digital sales infrastructure.
*   **Core Message**:
    *   Websites that convert.
    *   AI systems that save time.
    *   Automation that helps businesses scale.
*   **Trust Metrics**: Trusted by 100+ clients worldwide.

### Locations & Contact Info
*   **Website**: [https://www.arcai.agency](https://www.arcai.agency)
*   **Contact Page**: [https://www.arcai.agency/contact](https://www.arcai.agency/contact)
*   **Support Email**: support@arcai.agency (Responds within 24 hours)
*   **Phone Numbers**:
    *   **United Kingdom (UK)**: +44 7466 368427
    *   **Sri Lanka (SL)**: +94 771852522
*   **Physical Office (Sri Lanka)**: 4th Floor, 91 Daisy Villa Avenue, Colombo 4
*   **Physical Office (UK)**: Birmingham, UK
*   **Business Hours**: Monday to Friday (9:00 AM – 6:00 PM), Weekends (By appointment)

### Social Media
*   **X (Twitter)**: [https://x.com/arc_ai_agency](https://x.com/arc_ai_agency)
*   **Instagram**: [https://www.instagram.com/arcai_agency/](https://www.instagram.com/arcai_agency/)
*   **LinkedIn**: [https://www.linkedin.com/company/105845719](https://www.linkedin.com/company/105845719)
*   **Facebook**: [https://www.facebook.com/ARCAI.lk](https://www.facebook.com/ARCAI.lk)

### Official Links
*   **Booking Link**: [Book a 30-minute meeting](https://calendly.com/arcai-support/30min)
*   **WhatsApp Support**: [Direct Chat](https://wa.me/447466368427)
*   **Privacy Policy**: [https://www.arcai.agency/privacy-policy](https://www.arcai.agency/privacy-policy)
*   **Terms of Service**: [https://www.arcai.agency/terms-of-service](https://www.arcai.agency/terms-of-service)

---

## 2. Web Design & Development Packages

### A. Sri Lankan Rupee (LKR) Pricing
These are one-time payments with no ongoing monthly software fees.

#### 1. Starter Package — "Get Online Fast"
*   **Price**: Rs 60,000 (One-time payment)
*   **Timeline**: 2 – 3 days
*   **Best For**: Businesses that need a clean, professional web presence fast.
*   **What's Included**:
    *   Modern responsive website (5 pages)
    *   Clean, minimal design with standard layouts
    *   WhatsApp integration button + contact/inquiry form
    *   Mobile-optimized & fast-loading
    *   Free hosting forever (subject to 1GB media limit)
*   **What's Excluded**:
    *   Custom animations & parallax effects
    *   Glassmorphism & advanced UI
    *   Bespoke hero sections
    *   SEO structured data

#### 2. Launch Package — "Stand Out & Convert"
*   **Price**: Rs 90,000 (One-time payment)
*   **Timeline**: 3 – 5 days
*   **Best For**: Businesses that want a stunning, conversion-focused website that stands out.
*   **What's Included** (Everything in Starter, plus):
    *   Premium custom design with advanced animations
    *   Parallax scrolling, hover effects, and micro-interactions
    *   Glassmorphism cards & bespoke hero sections
    *   Video/animation support in hero
    *   Conversion-optimized layout with strategic Call-to-Actions (CTAs)
    *   Full SEO package with structured schema data & meta tags
*   **Upgrade Path**: Upgrade from Starter to Launch for a Rs 30,000 price difference.

#### 3. Growth Package — "Capture & Close Leads" (Most Popular)
*   **Price**: Rs 130,000 (One-time payment)
*   **Timeline**: 5 – 7 days
*   **Best For**: Businesses that want a premium website plus a system to capture, track, and close more leads.
*   **What's Included** (Everything in Launch, plus):
    *   Lead Dashboard — every website inquiry stored in one centralized database
    *   CRM pipeline tracker (New → Contacted → Quoted → Won/Lost)
    *   Email capture + newsletter integration
    *   Email marketing campaigns — templates for promos, updates, and re-engagements
*   **Upgrade Path**: Upgrade from Launch to Growth for a Rs 40,000 price difference.

#### 4. Scale Package — "24/7 AI-Powered Sales"
*   **Price**: Rs 160,000 (One-time payment) + $4/month AI fee
*   **Timeline**: 5 – 7 days
*   **Best For**: Businesses that want everything in Growth plus an AI agent handling inquiries around the clock.
*   **What's Included** (Everything in Growth, plus):
    *   AI conversational agent integrated directly into the website
    *   Instant responses to visitor questions 24/7 based on your knowledge base
    *   Handles common inquiries, captures contact details, and guides users to book a call
    *   Drastically reduces response time and captures after-hours traffic
*   **Upgrade Path**: Upgrade from Growth to Scale for a Rs 30,000 price difference + $4/month AI usage fee.

---

### B. British Pound (GBP) Pricing (United Kingdom)
These are one-time payments with no monthly fees unless specified.

#### 1. Starter Package — "Get Online Fast"
*   **Price**: £429 (One-time payment)
*   **Timeline**: 2 – 3 days
*   **Best For**: Small businesses needing a quick, clean, and professional online presence.
*   **What's Included**:
    *   Modern responsive website (5 pages)
    *   Clean, minimal design with standard layouts
    *   WhatsApp button + contact/inquiry form
    *   Mobile-optimized & fast-loading
    *   Free hosting forever (subject to 1GB media limit)
*   **What's Excluded**: Custom animations, parallax, glassmorphism, advanced UI, bespoke hero, SEO schema data.

#### 2. Launch Package — "Stand Out & Convert"
*   **Price**: £649 (One-time payment)
*   **Timeline**: 3 – 5 days
*   **Best For**: Businesses wanting a high-converting, stunning layout to stand out from competitors.
*   **What's Included** (Everything in Starter, plus):
    *   Standard 8-page website
    *   Premium custom design with advanced animations
    *   Parallax scrolling, hover effects, and micro-interactions
    *   Glassmorphism cards & bespoke hero sections
    *   Video/animation support in hero
    *   Conversion-optimized layout with strategic CTAs
    *   Full SEO with structured data & meta tags

#### 3. Growth Package — "Capture & Close Leads" (Most Popular)
*   **Price**: £999 (One-time payment)
*   **Timeline**: 5 – 7 days
*   **Best For**: Businesses seeking a premium site paired with automated systems to capture and follow up on leads.
*   **What's Included** (Everything in Launch, plus):
    *   Extensive responsive website (15 pages)
    *   Lead Dashboard — centralized storage for all inquiries
    *   CRM pipeline (New → Contacted → Quoted → Won/Lost)
    *   Email capture + newsletter system
    *   Email campaigns — promos, updates, and re-engagement flows

#### 4. Scale Package — "24/7 AI-Powered Sales"
*   **Price**: £1299 (One-time payment) + £4/month AI fee
*   **Timeline**: 5 – 7 days
*   **Best For**: Businesses requiring the full website framework plus an always-on AI receptionist.
*   **What's Included** (Everything in Growth, plus):
    *   Extensive responsive website (23 pages)
    *   AI conversational agent integrated into the website
    *   Instant responses to visitor questions 24/7
    *   Lead capture, qualification, and automated action guidance
    *   1 Month Free Keyword Optimization

*   **Additional Pages (UK)**: £50 per additional page.

---

### C. E-Commerce Development Tiers

#### E-Commerce (Shopify Theme Build)
*   **Setup Price**: Rs 75,000
*   **Timeline**: 5 – 7 days
*   **Ongoing Cost**: +$25/month Shopify subscription fee
*   **Includes**:
    *   Shopify theme setup & brand customization
    *   Product catalog setup & collection management
    *   Secure checkout flow
    *   Unlimited pages

#### E-Commerce (Custom Next.js Storefront)
*   **Setup Price**: From Rs 120,000 (Based on scale/requirements)
*   **Timeline**: 2 – 4 weeks
*   **Includes**:
    *   100% custom-coded storefront
    *   Bespoke animations & premium UI transitions
    *   Unrestricted SEO structures
    *   Unlimited pages & integrations

---

### D. Ad-Hoc Web Modifications & Integrations
*   **Additional Page Build (LKR)**: Rs 4,000 – Rs 6,000 per page
*   **Local Payment Gateway Integration**: Rs 25,000 one-time fee (Integration of PayHere or Webxpay for LKR payment processing)

---

## 3. AI & Automation Packages (Sri Lanka / LKR)

Our AI packages help businesses automate repetitive workflows, handle 24/7 inquiries, and scale without manual overhead.

### 1. Flow Package — "Automate Repetitive Work"
*   **Setup Fee**: Rs 75,000
*   **Monthly Retainer**: From Rs 10,000/month
*   **Deployment Timeline**: 1 – 2 weeks
*   **Best For**: Small businesses, service providers, and teams looking to cut administrative fatigue and automate basic processes.
*   **What's Included**:
    *   Automation for **one** core business process
    *   Automated data extraction (parsing receipts, forms, or documents)
    *   Automatic updates to Google Sheets, Airtable, or internal databases
    *   Alerts and notifications (Slack, Discord, WhatsApp) when action is required
    *   Simple internal AI assistant to help teams query documents fast
*   **Exclusions**: Website AI Chat Assistant, AI Voice Calling, Sales CRM Pipeline Integrations, Multi-Agent Orchestration.

### 2. Engage Package — "24/7 AI Lead Capture" (Most Popular)
*   **Setup Fee**: Rs 135,000
*   **Monthly Retainer**: From Rs 15,000/month
*   **Deployment Timeline**: 2 – 3 weeks
*   **Best For**: Businesses receiving regular web traffic that want a reliable way to capture and qualify inquiries instantly.
*   **What's Included** (Everything in Flow, plus):
    *   AI Chat Assistant embedded on your website
    *   Dynamic answers trained on your exact business knowledge base
    *   Intelligent lead capture and qualification (name, contact info, interest)
    *   Direct integration with appointment scheduling software (Calendly, etc.)
    *   Qualified lead alerts sent instantly to your team or CRM
    *   Performance analytics reporting dashboard
*   **Exclusions**: Inbound/Outbound AI Voice Calling, Automated follow-up SMS/Email sequences, Multi-Agent Orchestration.

### 3. Qualify Package — "AI Voice Follow-Up & Booking"
*   **Setup Fee**: Rs 195,000
*   **Monthly Retainer**: From Rs 24,000/month
*   **Deployment Timeline**: 3 – 5 weeks
*   **Best For**: High-lead-volume businesses needing immediate lead response times and automated outbound qualification.
*   **What's Included** (Everything in Engage, plus):
    *   AI Voice Assistant for handling inbound calls or launching immediate outbound calls to form submissions
    *   Automated lead qualification over the phone using custom voice models (e.g., ElevenLabs)
    *   Automated immediate SMS or email follow-up sequences based on call outcomes
    *   Catches call notes, summaries, and transcripts automatically
    *   Syncs all lead data directly to your database or CRM
*   **Exclusions**: Full enterprise-wide Revenue & Operations custom orchestration, custom pre-call lead research bots.

### 4. Command Package — "Full AI Revenue System"
*   **Setup Fee**: Rs 310,000
*   **Monthly Retainer**: From Rs 45,000/month
*   **Deployment Timeline**: 6 – 8 weeks
*   **Best For**: Growing mid-sized businesses, agencies, and high-ticket service companies requiring a unified, autonomous revenue engine.
*   **What's Included** (Everything in Qualify, plus):
    *   Unified AI chat and voice agents working in synchronization
    *   Multi-stage automated follow-up sequences across email, SMS, and WhatsApp
    *   Co-pilot sales tools (e.g., pre-call lead research briefs, competitive comparison generators)
    *   Custom API integrations with CRM, internal ERP, calendar, and billing workflows
    *   C-Suite executive reporting dashboards for analytics and conversion metrics
    *   Autonomous multi-agent orchestration for advanced operations

---

### E. Ongoing Retainer Details & Variable API Costs

#### Monthly Retainer
Covers server hosting, continuous uptime monitoring, prompt tuning, knowledge base synchronization (monthly updates), and API connector maintenance.
*   **Flow**: From Rs 10,000/mo
*   **Engage**: From Rs 15,000/mo
*   **Qualify**: From Rs 24,000/mo
*   **Command**: From Rs 45,000/mo

#### Variable AI Usage (API Cost)
AI token and voice model usage are billed **at cost** with zero markup. The client can link their credit card directly to OpenAI/ElevenLabs, or ARC AI passes the costs through.
*   **Average monthly usage**: Most SMBs spend between **Rs 2,000 and Rs 8,000 per month** depending on query volumes.

---

## 4. Digital Marketing & Social Media Pricing

These packages are offered under **Halo Media (Powered by ARC AI)**, specializing in monthly social media management, brand visibility, and target advertising campaigns.

### Monthly Retainer Packages

#### 1. Starter Package — "Establish Your Presence"
*   **Price**: LKR 50,000 / Month
*   **Best For**: Small businesses initiating their social media presence and looking for consistency.
*   **What's Included**:
    *   10 custom social media graphic posts per month
    *   Content planning, calendaring & scheduling
    *   Basic caption writing
    *   Basic graphic design support
    *   Monthly performance overview report
*   **Exclusions**: Mobile Reels/Short-form videos, advanced strategy, professional copy, detailed strategy reviews, brand workshops.

#### 2. Intermediate Package — "Accelerate Your Engagement" (Most Popular)
*   **Price**: LKR 80,000 / Month
*   **Best For**: Growing businesses looking for professional consistency paired with video engagement.
*   **What's Included** (Everything in Starter, plus):
    *   18 custom social media graphic posts per month
    *   Professional caption writing
    *   Priority graphic design support
    *   1 Mobile Reel/Short-form video per month (with trending audio selection and editing)
    *   Monthly performance report
*   **Exclusions**: Advanced strategy consultation, bespoke campaign ad copywriting, photography shoots, strategy workshops.

#### 3. Premium Package — "Dominate the Digital Space"
*   **Price**: LKR 120,000 / Month
*   **Best For**: Established brands needing comprehensive campaign strategies and premium multi-format content.
*   **What's Included** (Everything in Intermediate, plus):
    *   20 custom social media graphic posts per month
    *   Advanced content strategy development
    *   Professional copywriting & ad messaging
    *   2 Mobile Reels/Short-form videos per month
    *   Brand consultation & campaign planning workshops
    *   Detailed analytics, performance audit, and monthly strategy review
    *   Priority creative support

### Halo Media Add-Ons (LKR)
*   **Additional Creative Post**: Rs 15,000 / post (Custom static or carousel graphic)
*   **Additional Mobile Reel**: Rs 25,000 / reel (Up to 60s, subtitles, trending audio, color grading)
*   **Ads Setup & Management**: Rs 35,000 / month (Weekly optimization of Meta or Google Ads campaigns)
*   **Professional Photography**: Rs 35,000 / shoot (Half-day shoot on-location for team, product, or lifestyle assets)
*   **Brand Strategy Workshop**: Rs 40,000 / session (Deep-dive into buyer personas, tone of voice, visual guidelines)

---

## 5. Website Hosting & Maintenance

### Website Hosting
*   **Cost**: **FREE FOREVER**
*   *Note*: Includes free deployment and hosting as long as the total media storage stays under 1GB. Storage exceeding 1GB will be quoted separately.
*   *Domain*: Domain registration and annual renewal are handled directly by and billed to the client. Domains are NOT included in packages.

### Maintenance Retainers

#### 1. Annual Peace of Mind Retainer
*   **Price**: Rs 60,000 / Year
*   **Includes**:
    *   Security & dependency updates
    *   Uptime monitoring & automated backups
    *   Minor text/image modifications (reasonable monthly threshold)
    *   Bug fixes for existing features
*   **Exclusions**: Major redesigns, new features, or building new pages.

#### 2. Pay-Per-Fix Plan
*   **Price**: Rs 4,000 / Request
*   **Includes**:
    *   Fixing a specific bug or visual error
    *   Small UI styling tweaks
    *   Updating text or images on an existing page
    *   Resolving broken links/buttons/forms
*   **Exclusions**: Feature expansions or new page builds.

#### 3. Quarterly Website Maintenance
*   **Price**: Rs 40,000 / 3 Months (Standard recurring checkup and updates)

---

## 6. Portfolio & Completed Projects (50+ Projects)

The following database details completed and in-development projects deployed by ARC AI. 

### A. Live Production Sites

1.  **Imperial Life Sciences Pvt Ltd**
    *   **Website**: [https://www.imperiallifesciences.com](https://www.imperiallifesciences.com)
    *   **Description**: The website showcases Imperial Life Sciences’ healthcare distribution, medical solutions, and supply chain services through a professional, reliable, and modern digital experience built to strengthen trust and brand presence.
2.  **Inventis Pharma**
    *   **Website**: [https://www.inventispharma.org](https://www.inventispharma.org)
    *   **Description**: The website presents Inventis Pharma’s smart biomedical solutions, medical devices, rehabilitative equipment, and surgical consumables through a strong, professional, and healthcare-focused digital experience.
3.  **iConn Health Care**
    *   **Website**: [https://www.iconnhealthcare.org](https://www.iconnhealthcare.org)
    *   **Description**: The website presents iConn Health Care’s diagnostic and healthcare solutions through a clean, modern, and professional digital experience built to highlight quality, accuracy, and trusted medical support.
4.  **YARI**
    *   **Website**: [https://www.yariagency.com](https://www.yariagency.com)
    *   **Description**: A futuristic, high-performance web experience presenting digital automation, custom storefront options, web applications, and logistics solutions.
5.  **PluggedIn**
    *   **Website**: [https://www.pluggedin.lk](https://www.pluggedin.lk)
    *   **Description**: Modern, stylish, e-commerce-ready platform showcasing tech, electronics, personal gadgets, and lifestyle products.
6.  **TriMed Pharma**
    *   **Website**: [https://www.trimedpharma.com](https://www.trimedpharma.com)
    *   **Description**: Clean, professional, and trust-oriented digital showcase presenting healthcare and pharmaceutical distribution solutions.
7.  **Zenith Global Pvt Ltd**
    *   **Website**: [https://www.zenithglobal.biz](https://www.zenithglobal.biz)
    *   **Description**: Clinical engineering and medical equipment platform, highlighting critical care and healthcare tech solutions.
8.  **Lavendra Beauty Lounge**
    *   **Website**: [https://www.lavendrabeauty.lk](https://www.lavendrabeauty.lk)
    *   **Description**: Renowned beauty salon portal for booking personalized beauty, care, and aesthetic treatments.
9.  **Advitec International**
    *   **Website**: [https://advitecint.com](https://advitecint.com)
    *   **Description**: Trust-focused biomedical device distribution website representing top manufacturers across Sri Lanka.
10. **Portland Flooring**
    *   **Website**: [https://www.portlands.com.au/](https://www.portlands.com.au/)
    *   **Description**: High-end corporate site showcasing elegant, durable, and sustainable flooring solutions for the Australian market.
11. **Ceylon Tea Land**
    *   **Website**: [https://www.ceylonealand.com/](https://www.ceylonealand.com/)
    *   **Description**: Premium B2B export platform for Ceylon Tea Land (Pvt) Ltd, facilitating global wholesale tea inquiries directly from Sri Lankan tea estates.
12. **Java Global Nexus Platform**
    *   **Website**: [https://www.javagnp.ae](https://www.javagnp.ae)
    *   **Description**: High-performance corporate portal representing a global delivery and governance execution hub based in Dubai.
13. **Inspira Worldwide**
    *   **Website**: [https://inspiraworldwide.com](https://inspiraworldwide.com)
    *   **Description**: Professional service portal for a pharmaceutical distribution outsourcing and logistics partner in Sri Lanka.
14. **Agenly**
    *   **Website**: [https://www.agenly.online/](https://www.agenly.online/)
    *   **Description**: A modern SaaS platform enabling users to build, train, and deploy custom AI agents on their websites in minutes.
15. **TOS Lanka**
    *   **Website**: [https://www.toslanka.com](https://www.toslanka.com)
    *   **Description**: Corporate site for a pioneer Electronic Manufacturing Services (EMS) provider and BOI subsidiary of Tosslec (Japan).
16. **Mulberry Living**
    *   **Website**: [https://www.mulberry-living.com/](https://www.mulberry-living.com/)
    *   **Description**: Tailored boutique accommodation Negombo booking platform built on Next.js, featuring seamless online reservations.
17. **Java Global Access Platform LLC**
    *   **Website**: [https://javagap.ae/](https://javagap.ae/)
    *   **Description**: Global delivery tech/professional services portal. Integrated a custom CRM and customer support AI assistant.
    *   **Results**: Operational efficiency increased by 45%, support response times reduced by 60%, client management streamlined.
18. **Sponge Global (Pvt) Ltd**
    *   **Website**: [https://www.spongeglobal.com/](https://www.spongeglobal.com/)
    *   **Description**: Interactive learning and development solutions site (custom eLearning, immersive VR/AR training).
    *   **Results**: Training engagement up by 40%, course completion up by 35%, client inquiries up by 50%.
19. **Delft Tours & Travel (Pvt) Ltd**
    *   **Website**: [https://www.delfttours.com/](https://www.delfttours.com/)
    *   **Description**: Tourism booking site for immersive island tours around Sri Lanka, featuring interactive itinerary mapping.
    *   **Results**: Online tour bookings increased by 55%, itinerary requests up by 40%, user engagement up by 65%.
20. **Secretaries Consultant Services (Pvt) Ltd**
    *   **Website**: [https://consultantservices.lk/](https://consultantservices.lk/)
    *   **Description**: Clean corporate portal showcasing end-to-end legal and corporate secretary lifecycle services.
    *   **Results**: Operational efficiency up by 45%, corporate workflow speeds up by 40%, admin overhead down by 35%.
21. **Halo 360**
    *   **Website**: [https://halo360.online/](https://halo360.online/)
    *   **Description**: Sleek portfolio and landing page system for a partner digital marketing agency.
    *   **Results**: Brand visibility up by 45%, organic SEO traffic up by 60%, strategic partnership inquiries up by 35%.
22. **EXIM Corporation**
    *   **Website**: [https://eximcpl.com](https://eximcpl.com)
    *   **Description**: Industrial manufacturing and global partner network directory site.
    *   **Results**: Global network reach up by 40%, supply chain efficiency up by 25%, manufacturing inquiries up by 30%.
23. **Core Craft**
    *   **Website**: [https://www.corecraft.agency](https://www.corecraft.agency)
    *   **Description**: High-end creative agency portfolio showing premium UI/UX case studies. Integrated CRM and client workflows.
    *   **Results**: Lead generation increased by 45%, client tracking efficiency up by 60%.
24. **Ontriq**
    *   **Website**: [https://www.ontriq.com](https://www.ontriq.com)
    *   **Description**: Background verification and HR solutions provider portal, built with a custom case-tracking dashboard.
    *   **Results**: Client inquiries increased by 52%, processing turnaround time reduced by 30%.
25. **Orkestrate**
    *   **Website**: [https://www.orkestrate.com](https://www.orkestrate.com)
    *   **Description**: Modern marketing orchestration SaaS platform website built on Next.js.
26. **KeysPlease**
    *   **Website**: [https://www.keysplease.shop](https://www.keysplease.shop)
    *   **Description**: Lead generation-focused real estate marketplace for high-end residential listings in Sri Lanka.
    *   **Results**: Property inquiries up by 67%, CRM management efficiency up by 58%, listing views up by 3.2x, follow-up rates up by 72%.
27. **DEK Studio**
    *   **Website**: [https://www.dek-studio.com/](https://www.dek-studio.com/)
    *   **Description**: Trust-forward MEP (Mechanical, Electrical, Plumbing) engineering services site.
    *   **Results**: Lead volume increased by 31%, proposal requests up by 24%, 92% PageSpeed score.
28. **Vibe Web Studio**
    *   **Website**: [https://vibewebstudio.com](https://vibewebstudio.com)
    *   **Description**: Sales-first conversion design showcase with transparent packages.
    *   **Results**: Qualified leads increased by 1.9x, conversion rate up by +3.2%, 1.9s Largest Contentful Paint (LCP) speed.
29. **YBO Agency**
    *   **Website**: [https://yboagency.com](https://yboagency.com)
    *   **Description**: Creative back-office positioning with modular service selection blocks and instant quote selectors.
    *   **Results**: Brief submissions up by 27%, average deal value up by 14%, bounce rate reduced by 11%.
30. **Car Rush**
    *   **Website**: [https://www.carrush.lk/](https://www.carrush.lk/)
    *   **Description**: Responsive vehicle marketplace platform with custom search filtering.
    *   **Results**: Vehicle views up by 38%, lead-to-dealer CTR up by +2.6% (to 9.4%), 1,200 saved searches monthly.
31. **Car Arena Ceylon**
    *   **Website**: [https://www.cararenaceylon.com/](https://www.cararenaceylon.com/)
    *   **Description**: Automobile dealer hub showcasing warranties, OEM upgrades, and booking mechanics.
    *   **Results**: Bookings increased by 29%, LCP optimized to 2.2s.
32. **Hilltop Recruitment**
    *   **Website**: [https://hiltopglobal.com/](https://hiltopglobal.com/)
    *   **Description**: Heritage-driven recruitment platform with custom sector filters.
    *   **Results**: Candidate sign-ups increased by 23%, client inquiries up by 18%, organic impressions up by 41%.
33. **Island Jerky**
    *   **Website**: [https://www.islandjerky.lk/](https://www.islandjerky.lk/)
    *   **Description**: Clean, flavorsome e-commerce store with story-led flavor options.
    *   **Results**: Store conversion rate up by +0.7% (to 2.4%), AOV up by 12%.
34. **Digital Kade**
    *   **Website**: [https://digitalkade.lk/](https://digitalkade.lk/)
    *   **Description**: Fast-loading catalog with instant filtering and search index.
    *   **Results**: Search-to-purchase up by +2.1% (to 6.3%), total sales revenue up by 16%.
35. **Jacob Day Coaching**
    *   **Website**: [https://www.jacobday.co/](https://www.jacobday.co/)
    *   **Description**: Clean landing page for personal development, showcasing high-impact social proof widgets.
    *   **Results**: Consultation calls up by 26%, refund rates decreased to a minimal 1.8%.
36. **Mouttaki Coaching**
    *   **Website**: [https://mouttakicoaching.com/](https://mouttakicoaching.com/)
    *   **Description**: High-impact personal trainer portfolio site, showcasing client transformation sliders.
    *   **Results**: Lead-to-client conversion up by +3.8% (to 12.5%), NPS rating of 64.
37. **ARC AI CRM System**
    *   **Description**: Custom internal system developed to streamline lead capture, client onboarding, and automated email trigger sequences.
    *   **Results**: Successfully unified CRM and lead logs.

### B. In Development / Staging Projects

38. **Fit Bite**
    *   **Website**: [https://www.fitbite.shop/](https://www.fitbite.shop/)
    *   **Description**: Artisan protein cookie store with customizable nutritional settings.
    *   **Results**: Upfront online sales increased by 21%, repeat purchases at 24%.
39. **Inlaid Jewelry**
    *   **Website**: [https://cosmic-dusk-c32013.netlify.app/](https://cosmic-dusk-c32013.netlify.app/)
    *   **Description**: Heritage jewelry platform showcasing Tamil culture-aligned custom aesthetics.
    *   **Results**: Custom consults up by 28%, average session value up by 13%.
40. **Hijab Boutique**
    *   **Website**: [https://hijabboutiquestore.netlify.app/](https://hijabboutiquestore.netlify.app/)
    *   **Description**: Modest apparel storefront utilizing optimized mobile catalogs.
    *   **Results**: Checkout conversions up +0.9%, cart abandonment down by 6%.
41. **Joe Fitness**
    *   **Website**: [https://mellifluous-jalebi-dd6aef.netlify.app/](https://mellifluous-jalebi-dd6aef.netlify.app/)
    *   **Description**: Simple lead-generation landing page for local fitness trainers.
    *   **Results**: Lead capture rate of 38%, email open rate at 41%.
42. **Essential Marketing**
    *   **Website**: [https://essentialmarketing.netlify.app](https://essentialmarketing.netlify.app)
    *   **Description**: Studio site turning agency briefs into bookable packages.
    *   **Results**: Proposal acceptance rate up to 38%, response times cut by 19%.
43. **Ceylon Motors**
    *   **Website**: [https://ceylonmotors.netlify.app](https://ceylonmotors.netlify.app)
    *   **Description**: Discovery and directory engine for buy/rent car models.
    *   **Results**: Filtered searches up by 33%, average session length at 3:04.
44. **Bakery Website**
    *   **Website**: [https://prismatic-mooncake-640ca9.netlify.app](https://prismatic-mooncake-640ca9.netlify.app)
    *   **Description**: Immersive, animated layout demonstrating modern Next.js styling.

### C. Live Custom Design Templates
These links host interactive demos deployed for client previews:
45. **Luxury Restaurant**: [Demo Link](https://luxaryrestaurant-demo.netlify.app/)
46. **Travel Agency**: [Demo Link](https://beautiful-conkies-2df92d.netlify.app/)
47. **Real Estate**: [Demo Link](https://silly-bombolone-0213cb.netlify.app/)
48. **Corporate Website**: [Demo Link](https://courageous-fenglisu-1afb0d.netlify.app/)
49. **Music/Radio Website**: [Demo Link](https://music-demo1.netlify.app/)
50. **Travel/Tourism**: [Demo Link](https://delftdemo.netlify.app/)

---

## 7. Core FAQs & Technical Guidance

### Q: What is the main difference between legacy WordPress and Next.js Headless architectures?
*   **WordPress (Monolithic)**: The frontend user interface and backend database are tightly bound. Every page load triggers server-side queries to compile the page dynamically. This results in dynamic database latency, plugin vulnerability risks (Plugin Rot), and slower Time-To-First-Byte (TTFB) scores.
*   **Next.js (Headless)**: The frontend is fully decoupled. Pages are pre-compiled into static HTML/React during the build phase and distributed globally via CDNs (e.g., Vercel, Netlify). The frontend calls the database/CMS strictly via secure APIs. This makes the site impervious to SQL injections and enables instant, sub-second page loads.

### Q: What are Google's Core Web Vitals targets?
Any professional corporate site must pass Google's three key telemetry checks for organic ranking:
1.  **LCP (Largest Contentful Paint)**: Measures load speed. Must render the main above-the-fold content in **less than 2.5 seconds**.
2.  **INP (Interaction to Next Paint)**: Measures user responsiveness. The page must respond to clicks or taps in **under 200 milliseconds**.
3.  **CLS (Cumulative Layout Shift)**: Measures visual layout stability. Layout shifts due to loading elements must maintain a score **under 0.1**.

### Q: How do we build multilingual sites for Sri Lankan companies?
We build local sites using Next.js native `i18n` internationalization support. This enables fast, clean transitions between English, Sinhala, and Tamil. The framework uses proper Unicode font rendering for local scripts and structures SEO tags (`hreflang`) so search bots direct users to the appropriate script.

### Q: What industries does ARC AI serve?
We build tailored digital assets across all major verticals:
*   Tourism & Hospitality (with multilingual booking engines)
*   Real Estate (with listing filtration maps)
*   Healthcare (with HIPAA-conscious portals)
*   Education & LMS Systems
*   Industrial Manufacturing (with RFQ capabilities)
*   Fintech, Food & Beverage, BOI enterprises, and more.

### Q: What are the exact upgrade paths for Website Packages?
We credit whatever the client has already paid toward the higher tier:
*   **Starter to Launch**: Rs 30,000 difference (Upgrades to premium custom design & SEO).
*   **Launch to Growth**: Rs 40,000 difference (Adds Lead Dashboard & CRM pipeline).
*   **Growth to Scale**: Rs 30,000 difference + $4/mo (Adds conversational AI agent).

---
